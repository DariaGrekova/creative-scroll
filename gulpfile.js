...
const buildFolder = 'static'; //папка куда собирается проект (указываем docs, если нужен gitHubPage, дополнительно нужно указать в настройках gitHub)
...
function libs() {
    return src(sourceFolder + '/libs/**/*')
        .pipe(dest(buildFolder + '/libs'))
};
...
function serve() {
    ...
    watch(sourceFolder + '/libs/**/*', series(libs)).on('change', sync.reload)

};


exports.build = series(clear, scss, js, img, sprite, fonts, libs, html);
exports.watch = series(clear, scss, js, img, svg, sprite, fonts, libs, html, serve);
exports.bem = bem;
exports.clear = clear;